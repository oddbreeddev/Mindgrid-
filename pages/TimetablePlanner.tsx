  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 md:py-12">
      <div className="flex flex-col gap-6 mb-8 md:mb-12">
        <h1 className="text-3xl md:text-4xl font-black text-slate-800">Study <span className="text-blue-600">Planner</span></h1>
          ...
          <div className="flex lg:flex-col overflow-x-auto no-scrollbar gap-2 py-2">
            {DAYS.map(day => (
              <button
                ...
                className={`flex-shrink-0 px-5 py-3 md:py-4 md:px-6 rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest transition-all text-center ${activeTab === day ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white text-slate-400 border border-slate-100'}`}
              >
                {day.substring(0, 3)}
              </button>
            ))}
        ...
        <div className="lg:col-span-9">
          <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-slate-100 shadow-sm min-h-[400px]">
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => addSession(activeTab)} className="text-blue-600 text-[10px] font-black uppercase flex items-center gap-2"><i className="fas fa-plus"></i> Add</button>
            </div>
            <div className="space-y-4">
              {currentDayPlan?.sessions.length === 0 ? (
                ...
                currentDayPlan?.sessions.map((session, idx) => (
                  <div key={idx} className="bg-slate-50 p-4 rounded-2xl border border-transparent hover:border-blue-100 flex flex-col md:flex-row gap-3">
